...
async function findContractByToken(token: string): Promise<{ contract: Record<string, unknown>; signer: SignerInfo } | null> {
  ...
  const primary = await supabase
    .from("contracts")
    .select("id, title, content, pages, payment_milestones, status, client_email, client_signed_at, owner_signed_at, additional_signers")
    .eq("sign_token", token)
    .maybeSingle();
  if (primary.data) {
    return {
...
async function signContract(req: VercelRequest, res: VercelResponse) {
  ...
  if (fullContract?.org_id) {
    ...
    sendPushToOrg(fullContract.org_id as string, {
      ...
      data: { type: "contract" },
    }).catch(err => console.warn(`[contract-sign] push failed: ${errorMessage(err)}`));
